...
function statusStyle(status: string) {
  return STATUS_STYLE[status] ?? { bg: "bg-[#F3F4F6]", text: "text-[#4B5563]", label: status };
}
...
export default function OperatorCalendarPage() {
  ...
  const { schedules, isLoading, error, refetch } = useOperatorSchedules(selectedKey);
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] font-outfit">
      <header className="sticky top-0 z-40 bg-white px-5 pt-3 pb-4 border-b border-gray-200 shadow-sm">
        <h1 className="text-[22px] font-bold text-[#111827]">Calendário de Viagens</h1>
        <p className="text-sm text-gray-500">Consulta de viagens por dia</p>
      </header>
      ...
      <main className="px-5 py-6 pb-28">
        <Card className="border-[#E5E7EB] mb-6 bg-white">
          <CardContent className="p-4">
            <Calendar
              ...
              classNames={{ day: cn("data-[selected-single=true]:rounded-full") }}
        ...
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-bold text-[#111827]">
            Partidas de {selected.getDate()} {MONTHS[selected.getMonth()]}
          </h3>
        </div>
            ...
            {schedules.map((schedule) => {
              ...
              return (
                <Card
                  ...
                  onClick={() => navigate(`/operator/manifest?schedule=${schedule.schedule_id}&date=${selectedKey}`)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      e.preventDefault();
                      navigate(`/operator/manifest?schedule=${schedule.schedule_id}&date=${selectedKey}`);
                    }
                  }}
                >
                  <CardContent className="px-4 py-2 flex justify-between items-center">
                    <div className="flex flex-col items-baseline justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2 text-gray-400">
                        <span className="text-[13px] font-inter text-[#1B7A3D] font-semibold">{schedule.departure_time}</span>
                        <span className="text-[#4B5563]">·</span>
                        <span className="text-[11px] font-inter font-normal text-[#4B5563]">
                          {schedule.available_seats}/{schedule.total_seats} livres
                        </span>
                      </div>
                    </div>
                    <span className={`text-[11px] font-semibold px-2.5 py-0.5 font-inter rounded-full ${style.bg} ${style.text}`}>
                      {style.label}
                    </span>
                  </CardContent>
                </Card>
        ...
        {!isLoading && !error && schedules.length === 0 && (
          <div className="text-center py-10 text-gray-400 text-sm">Nenhuma viagem neste dia</div>
        )}
      </main>
    </div>
  );
